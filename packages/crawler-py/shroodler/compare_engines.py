...
def merge_engine_results(py_doc: dict, go_doc: dict) -> dict:
    ...
    for engine, doc in (("python", py_doc), ("go", go_doc)):
        for f in doc.get("findings", []):
            ...
            if f.get("severity") != entry.get("severity"):
                entry["severity_disagreement"] = {
                    "python": f.get("severity") if engine == "python" else entry.get("severity"),
                    "go": f.get("severity") if engine == "go" else entry.get("severity"),
                }
