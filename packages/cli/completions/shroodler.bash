...
_shroodler_flags_for() {
    case "$1" in
        crawl)
            echo "--profile --mode --depth --max-pages --max-time --output --format --ignore-robots --no-sitemap --allow-external --check-rate-limit --no-check-rate-limit --check-subresources --header --user-agent --cookie --cookie-jar --storage-state --login-recipe --proxy --seed --seed-from --cookies-from"
            ;;
        diff)
