...
_shroodler_flags_for() {
    case "$1" in
        ...
        program-scope)
            ...
            ;;
        agent)
            echo "--program --target --max-iterations --max-pages-per-crawl --login-recipe --peer-recipe --reauth-max-retries --higher-priv-jar --lower-priv-jar --owner-cookie --peer-cookie --dry-run --llm-triage --llm-agent --llm-provider --llm-agent-model --llm-model --llm-agent-max-cost --run-discovery --write-authz-spec --ignore-robots --run-probes --no-openapi --no-ssrf --no-open-redirect --no-host-header --no-ssti --no-xxe --no-graphql --no-auto-register --reprobe --run-diff --llm-business-logic --chain-spec --dom-xss --no-crlf --no-prototype-pollution --no-content-discovery --no-tls-check --no-rate-limit-check --no-mass-assignment --smuggling --no-smuggling --no-websocket --no-js-analysis --allow-external --scope-file"
            ;;
        dedup)
            echo "--output"
            ;;
